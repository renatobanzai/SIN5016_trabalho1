...
def kfold_cross_validation(dt_obj):
    logging.info("ini kfold_cross_validation")
    ini_ = datetime.datetime.now()


        # x_test = X_all_folds[k]
        # y_test = Y_all_folds[k]
        #
        # x_train = X_all_folds[np.delete(folds, k)]
        # y_train = Y_all_folds[np.delete(folds, k)]
